...
func TestAzureDataFactory(t *testing.T) {
	...
	expectedDFName := terraform.Output(t, terraformOptions, "data_factory_name")
	expecctedRGName := strings.ToLower(terraform.Output(t, terraformOptions, "resource_group_name"))

	exec.Command("az", "extension", "add", "--name", "datafactory").Run()
	// Verify if the Data Factory exists in Azure using the az CLI
	cmd := exec.Command("az", "datafactory", "show", "--name", expectedDFName, "--resource-group", expecctedRGName)
	...
	assert.Equal(t, expectedDFName, actualName)
